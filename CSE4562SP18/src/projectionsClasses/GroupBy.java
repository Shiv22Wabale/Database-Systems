...
public class GroupBy extends ProjectionBase {
	...
	public GroupBy(TreeNode child, List<Column> columns, List<SelectItem> items ) {
		super(child, items);
	...
	public TupleRecord nextTuple(Column primary, PrimitiveValue condition, boolean once) {
		...
		try {
			...
			if(firstTime) {
				...
				List<PrimitiveValue> groubyValues;
...
//					System.err.println(" In group by " + tuple.getAll());
					
					groubyValues = new ArrayList<>(); 
					for(Column column : groupByList) {
						groubyValues.add(eval.eval(column));
					}
							...
							if( expression instanceof Function){
								...
								if(function.isAllColumns()) {
									...
									for (Expression aggregate : function.getParameters().getExpressions()) {
										//int i = 0;
//										for(Column column : schema) {
//											System.out.print( " " +column.getWholeColumnName());
//										}
//										System.out.println("\n   "+tuple.getAll());
//										System.out.println(function.getName().toUpperCase() + " for " + aggregate + " result is " + eval.eval(aggregate));
										//System.out.println(aggregate instanceof Multiplication);
